...
! grep -Fq '# CONFIG_KERNEL_XDP_SOCKETS is not set' "$compose"
grep -Fq '# CONFIG_KERNEL_MPTCP is not set' "$compose"
...
grep -Fq 'OPENWRT_DIR: /mnt/workdir/openwrt' "$shard_workflow"
grep -Fq 'scripts/validate-repository.sh' "$shard_workflow"
grep -Fq 'git clone --depth=1 --single-branch --branch "$SOURCE_BRANCH" "$FORK_REPO" "$OPENWRT_DIR"' "$shard_workflow"
grep -Fq 'working-directory: ${{ env.OPENWRT_DIR }}' "$shard_workflow"
grep -Fq 'scripts/build-feature-bundle.sh "$OPENWRT_DIR"' "$shard_workflow"
grep -Fq "'\${{ inputs.scope }}'" "$shard_workflow"
! grep -Fq '/workdir/openwrt' "$shard_workflow"
grep -Fq 'key: okwrt-v2-' "$shard_workflow"
! grep -Fq 'restore-keys:' "$shard_workflow"
...
# WHNAT can be enabled without MAP/MWDS, so the vendor routing table and its
# declaration do not exist.  Keep the roaming cleanup behind the same guard.
grep -Fq -- '--- a/mt_wifi/embedded/fsm/ap_mgmt_assoc.c' "$mtk_ap_patch"
grep -Fq '+#ifdef ROUTING_TAB_SUPPORT' "$mtk_ap_patch"
grep -Fq '+#endif /* ROUTING_TAB_SUPPORT */' "$mtk_ap_patch"
...
grep -Fq 'scope="${11:-all}"' "$bundle"
grep -Fq '[[ "$scope" == all || "$feature" == "$highest" ]] || continue' "$bundle"
grep -Fq 'scripts/verify-build-output.sh" "$out" $device_list' "$bundle"
...
grep -Fq -- "-path '*/ipkg-*/dumpimage'" "$bundle"
grep -Fq 'dumpimage_package_pending=true' "$bundle"
...
grep -Fxq 'CONFIG_KERNEL_XDP_SOCKETS=y' "$kernel_config_fixture/kernel-6m/.config"
grep -Fxq '# CONFIG_KERNEL_MPTCP is not set' "$kernel_config_fixture/kernel-6m/.config"
